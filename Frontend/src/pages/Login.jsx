  ...
  return (
    <main className="grid min-h-[calc(100vh-74px)] place-items-center px-4 py-10">
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-panel p-8 card-line shadow-glow">
        <p className="text-xs font-bold uppercase tracking-[0.35em] text-gold">Welcome back</p>
        <h1 className="mt-3 font-display text-4xl font-bold">Login</h1>
        <p className="mt-2 text-muted">Login to post items and manage claims.</p>
        ...
        <button disabled={loading} className="btn-gold mt-6 w-full">{loading ? "Logging in..." : "Login"}</button>
        <p className="mt-5 text-center text-sm text-muted">New here? <Link className="text-gold" to="/register">Create account</Link></p>
      </form>
    </main>
  );
}
